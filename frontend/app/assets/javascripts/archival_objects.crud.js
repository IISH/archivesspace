...
function TreeLinkingModal(config) {
  ...
  self.large_tree = new LargeTree(
    ...
    function () {
      ...
      self.$container.on(
        'click',
        '.table-row.largetree-node input',
        function (e) {
          e.stopPropagation();
          let uri = $(this).val();
          $('#addSelectedButton').removeClass('disabled');
          $('.largetree-node.current').removeClass('current');
          $(this).closest('.largetree-node').addClass('current');
          self.selected_row = $(this).closest('.largetree-node');
          self.large_tree.expandNode(self.selected_row, function () {
            ...
            inserted_row.addClass(
              'largetree-node indent-level-' +
                (self.selected_row.data('level') + 1)
            );
            ...
            if (
              ...
              inserted_row
                .find('.title')
                .append(' -- <em>drag to change position</em>');
            } else {
              // this would be an only child of the parent
              self.selected_row.after(inserted_row);
            }
